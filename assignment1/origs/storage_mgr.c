...
RC readBlock (int pageNum, SM_FileHandle *fHandle, SM_PageHandle memPage) {
	printf("Page number is %d", pageNum);
...
RC writeBlock (int pageNum, SM_FileHandle *fHandle, SM_PageHandle memPage) {
	printf("writing %s %d", (char *)memPage, (int)strlen((char *)memPage));
	FILE *fd = (FILE *)fHandle->mgmtInfo;
	if ( pageNum > fHandle->totalNumPages -1 || pageNum < 0) {
		return RC_READ_NON_EXISTING_PAGE;
	}
	if (check_fd_in_list(fHandle) == RC_OK) {
		if (fseek(fd, pageNum*PAGE_SIZE*sizeof(char), SEEK_SET) == 0) {
			if (fHandle->totalNumPages == 1) {
				char content[PAGE_SIZE] = {0};
				fread(content, sizeof(char), PAGE_SIZE, fd);
				/* For null content open the file in w+ to replace null's */
				if (content[0] == '\0')
					freopen(NULL, "w+", fd);
